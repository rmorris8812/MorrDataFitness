...
namespace IdentityServer
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Login(LoginInputModel model, string button)
        {
            ...
            if (button == "findUser")
            {
                var foundUser = await _userRepository.GetUserByEmailAsync(model.Username, CancellationToken.None);
                if (foundUser != null && foundUser.ExternalAuth)
                {
                    return RedirectToAction("Challenge", "External", new { provider = "Salesforce", model.ReturnUrl });
                }
                var viewModel = await BuildLoginViewModelAsync(model);
                viewModel.EnableLocalLogin = true;
            ...
            if (ModelState.IsValid)
            {
                ...
                var user = await _userRepository.GetUserByEmailAsync(model.Username, CancellationToken.None);
